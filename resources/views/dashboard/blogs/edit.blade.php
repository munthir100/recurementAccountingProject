...
@section('title')
@include("dashboard.layouts.shared.includes.title-meta", ["title" => __("Edit Blog Post")])
@endsection

@section('page-title')
@include("dashboard.layouts.shared.includes.page-title", ["pagetitle" => __("Account"), "title" => __("Edit Blog Post")])
@endsection
...
<div class="row">
    <div class="card">
        <div class="card-body">
            <form id="blogForm" action="{{ route('user.dashboard.blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <input type="hidden" id="context" name="context" value="{{ old('context', $blog->context) }}">
                ...
                @php
                $blogStatusValue = $blog->status_id == App\Models\Status::PUBLISHED ? 0 : 1 ;

                @endphp
                <!-- Is Published Checkbox -->
                <div class="form-group mb-2">
                    <input type="checkbox" id="is_published" name="is_published" value="{{$blogStatusValue}}" {{ $blog->status_id == App\Models\Status::PUBLISHED ? 'checked' : '' }}>
                ...
                <!-- Submit Button -->
                <button type="button" id="submitButton" class="btn btn-primary mt-4">{{ __('Update') }}</button>
            </form>
        </div>
    </div>
</div>
