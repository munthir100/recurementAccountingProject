            ...
            <div class="card-header">
                ...
                <div class="row g-4 mb-3">
                    <div class="col-sm-auto">
                        <div>
                            <a data-bs-toggle="modal" data-bs-target="#addcustomer" class="btn btn-success add-btn" id="create-btn">
                ...
                <div class="table-responsive table-card">
                    <table class="table table-nowrap table-striped-columns mb-3">
                        ...
                        <tbody>
                            ...
                            <form id="form{{ $customer->id }}" action="{{ route('user.dashboard.customers.destroy', $customer->id) }}" method="post" class="hidden">
                                @csrf
                                @method('delete')
                            </form>
                            <tr>
                                <td><a href="#" class="fw-semibold">{{ $customer->id }}</a></td>
                                <td>{{ $customer->account->name }}</td>
                                <td>
                                    <div class="d-flex gap-2">
                                        <div class="edit">
                                            <a href="{{ route('user.dashboard.customers.edit', $customer->id) }}" class="btn btn-icon btn-primary edit-item-btn" title="{{__('Edit')}}"><i class="bx bx-edit"></i></a>
                                        </div>
                                        <div class="remove">
                                            <button class="btn btn-icon btn-danger remove-item-btn" title="{{__('Delete')}}" data-id="{{ $customer->id }}"><i class="bx bx-trash"></i></button>
                                        </div>
                                    </div>
                                </td>
                            </tr>
                                        ...
                                        <div class="modal-body">
                                            <div class="mt-3">
                                                ...
                                                <div class="mt-4 pt-2 fs-15 mx-5">
                                                    ...
                                                    <p class="text-muted mx-4 mb-0">{{ __("Are you Sure You want to Delete this customer?") }}</p>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ __("Cancel") }}</button>
                                            <button type="button" class="btn btn-danger" onclick="event.preventDefault(); document.getElementById('form{{ $customer->id }}').submit();">
                                                {{ __("Delete") }}</button>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="d-flex justify-content-center">
                        <x-dashboard.pagination :model="$customers" />
                    </div>
                    <div class="modal fade" id="addcustomer" tabindex="-1" aria-labelledby="addcustomerLabel">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form action="{{ route('user.dashboard.customers.store') }}" method="POST">
                                        @csrf
